...
export default function App() {
  ...
  if (siteSettings.isComingSoonActive && currentView !== 'admin') {
    return (
      <div className={`min-h-screen flex flex-col justify-between relative overflow-hidden font-sans ${darkMode ? 'dark bg-neutral-900 text-white' : 'bg-royal-purple text-white'}`}>
        ...
        <AnimatePresence>
          {toastMessage && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -25 }}
              className={`fixed top-8 left-4 right-4 md:left-auto md:right-8 z-50 p-4 rounded-xl shadow-xl flex items-center gap-3 border bg-emerald-50 border-emerald-200 text-emerald-800`}
            >
        ...
        {/* Top bar with Dark Mode toggle and optional simple admin entry button if they are unlocked */}
        <div className="p-4 flex justify-between items-center z-10">
          <button
            onClick={() => setDarkMode(!darkMode)}
            className="w-10 h-10 rounded-2xl bg-white/10 hover:bg-white/20 text-brand-gold border border-white/10 flex items-center justify-center transition-all cursor-pointer"
            title="تغيير المظهر"
          >
            {darkMode ? '☀️' : '🌙'}
          </button>
          ...
          {isAdminUnlocked && (
            <button
              ...
              className="px-4 py-2 bg-brand-gold hover:bg-brand-gold-light text-royal-purple text-xs font-bold rounded-xl transition-all cursor-pointer"
            >
              لوحة التحكم ⚙️
            </button>
          )}
        ...
        {/* Center content */}
        <div className="flex-grow flex flex-col justify-center items-center px-4 py-12 relative z-10 text-center">
          ...
          >
            ...
            {siteSettings.logoUrl && (
              <div className="relative group cursor-pointer" onClick={handleFooterSecretClick}>
                <div className="absolute -inset-1.5 bg-gradient-to-r from-brand-gold to-amber-500 rounded-full blur-md opacity-40 group-hover:opacity-75 transition duration-500" />
                <img
                  src={siteSettings.logoUrl}
                  alt={siteSettings.storeName}
                  referrerPolicy="no-referrer"
                  className="relative w-32 h-32 md:w-40 md:h-40 rounded-full object-cover border-2 border-brand-gold shadow-lg"
                />
              </div>
            )}

            <div className="space-y-2.5">
              <span className="text-xs md:text-sm font-semibold tracking-widest text-brand-gold uppercase block animate-pulse-slow">
                {siteSettings.storeName || "Douaa & Basma"}
              </span>
              <h1 className="text-3xl md:text-5xl font-display font-black text-white text-center leading-tight">
                {siteSettings.comingSoonTitle || 'انتظرونا... نفتتح قريباً!'}
              </h1>
              <div className="w-24 h-1 bg-gradient-to-r from-transparent via-brand-gold to-transparent mx-auto my-4 rounded-full" />
              <p className="text-sm md:text-base text-gray-200 font-semibold max-w-lg mx-auto leading-relaxed">
                {siteSettings.comingSoonSubtitle || 'نعمل بحب وشغف لتجهيز أفخر العصائر والتحليات المنزلية تليق بضيافتكم.'}
              </p>
            </div>

            {/* Social media links */}
            <div className="pt-6 space-y-4">
              <span className="text-xs text-gray-300 font-bold block">تابعونا لتكونوا أول من يعلم عند الافتتاح:</span>
              <div className="flex gap-4 justify-center">
                <a
                  href={`https://wa.me/${siteSettings?.whatsappNumber || '212705908383'}`}
                  target="_blank"
                  rel="noreferrer"
                  className="w-12 h-12 rounded-2xl bg-[#25D366] hover:bg-[#20ba5a] text-white flex items-center justify-center transition-transform hover:scale-110 shadow-lg shadow-[#25D366]/20"
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-yellow-500 via-pink-500 to-purple-600 text-white flex items-center justify-center transition-transform hover:scale-110 shadow-lg shadow-pink-500/20"
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="w-12 h-12 rounded-2xl bg-blue-600 hover:bg-blue-500 text-white flex items-center justify-center transition-transform hover:scale-110 shadow-lg shadow-blue-600/20"
        ...
        {/* Footer info */}
        <div className="p-6 text-center text-[11px] text-gray-400 z-10 border-t border-white/5 bg-black/10">
          <p>© {new Date().getFullYear()} {siteSettings?.footerCredits || "جميع الحقوق محفوظة لعلامة"} <span className="font-bold text-gray-300 cursor-pointer" onClick={handleFooterSecretClick}>{siteSettings?.storeName || "Douaa & Basma"}</span></p>
        </div>
      </div>
    );
  }
